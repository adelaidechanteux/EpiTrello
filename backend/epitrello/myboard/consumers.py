...
from myboard.utils import (
    does_user_access_board,
    connect_user_board,
    disconnect_user_board,
)
...
class BoardRealTime(WebsocketConsumer):
    ...
    def connect(self) -> None:
        self.board_id = UUID(
            ...
            .get("kwargs", {})
            .get("board_id", "No Board ID provided"),
            version=4,
        )
    ...
    def receive(
        ...
        if text_data_json["type"] == "login" and "Authorization" in text_data_json:
                    ...
                    text_data=json.dumps(
                        {
                            "type": "login",
                            "success": False,
                            "code": "BoardPermission",
                            "error": "User is not in the board members",
                        }
                    )
                )
                return
            ...
            self.send(
                text_data=json.dumps(
                    {"type": "login", "success": True, "connected": connected}
                )
            )
            return
