...
def send_email_task_near_date_end(self):
    ...
    nb_send = 0
    tasks = Task.objects.filter(date_end__gte=(timezone.now() + timedelta(days=1)), one_day_email_send=False)
    for task in tasks:
        ...
        if len(boards) == 0:
            continue
        send_mail("EpiTrello | A Task is near the End", f"The task '{task.title}' in the board '{boards[0].title}' that is assigned to you, is supposed to finish in 1 day.", settings.EMAIL_SENDER, [f"{target.email}"])
