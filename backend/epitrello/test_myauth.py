...
class MyBoardTest(TestCase):
    def test_user_login(self):
        ...
        self.assertEqual(200, response.status_code, f"Get user failed | {response.text} | {response.status_code} | {response.headers}")
        res = response.json()
        ...
        self.assertEqual(200, response2.status_code, f"Get user failed | {response2.text} | {response2.status_code} | {response2.headers}")
        res2 = response2.json()
