      ...
      <template #item="{ element: column }">
        <div class="min-w-[260px] bg-(--ui-black) rounded-xl p-3 flex flex-col shadow hover:shadow-lg">
          <div class="flex items-center justify-between mb-3 pl-3 group ">
            <h3 class="font-semibold overflow-scroll" >{{ column.title }}</h3>
          ...
          <div class="mt-2 flex flex-col gap-1">
            ...
            <div v-else class="flex flex-col gap-1">
              <UInput v-model="newCardTitles[column.id]" maxlength="49" placeholder="Enter a title" color="info" :ui="{ base: 'bg-[var(--secondary-grey)] ring-0 rounded-lg pb-8 text-text-color' }" @keyup.enter="addCard(column)"/>
      ...
      <template #footer>
        <div class="min-w-[260px] flex-shrink-0">
          ...
          <div v-else class="flex flex-col gap-1 bg-(--ui-black) rounded-xl p-3">
            <UInput v-model="newColumnTitle" placeholder="Enter list title..." maxlength="49" color="info" :ui="{ base: 'bg-[var(--secondary-grey)] ring-0 rounded-lg text-text-color' }" @keyup.enter="addColumn"/>
            <div class="flex gap-2 mt-1">
